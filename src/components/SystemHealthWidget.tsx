...
export default function SystemHealthWidget() {
  ...
  const checkHealth = async () => {
    ...
    // Check MongoDB Health via Backend Endpoint
    const mongoStart = performance.now();
    let mongoConnected = false;
    let mongoLatency = 0;
    let mongoError: string | null = null;
    let mongoConnStr = '';
    let mongoEngine = 'MongoDB Atlas Cluster';
    ...
    try {
      ...
      if (res.ok) {
        ...
        mongoConnStr = data.connectionString || 'mongodb+srv://...';
        mongoEngine = data.engine || (mongoConnected ? 'MongoDB Atlas Cluster' : 'In-Memory Fallback');
        collectionsCount = data.collectionsCount || 0;
        totalRecords = data.totalRecords || 0;
        mongoError = data.lastError || (mongoConnected ? null : 'MongoDB server selection timed out');
      } else {
  ...
  const overallHealthy = health.mongo.connected;
  const isTesting = health.mongo.testing;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-slate-900 text-sm tracking-tight">MongoDB Database & Engine Health</h3>
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase flex items-center gap-1 border ${
                ...
                {overallHealthy ? 'Connected & Operational' : 'Connecting / Degraded'}
              </span>
            </div>
            <p className="text-xs text-slate-500 font-medium mt-0.5">Persistent document store telemetry, latency, and collection stats.</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {isTesting ? 'Checking...' : 'Ping MongoDB'}
          </button>
          <button
      ...
      <div className={`p-4 rounded-xl border transition-all ${
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <span className="font-bold text-slate-900 text-xs block leading-tight">{health.mongo.engine}</span>
              <span className="text-[10px] font-mono text-slate-500 truncate max-w-[280px] block" title={health.mongo.connectionString}>
          ...
          <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase flex items-center gap-1 border ${
            ...
            {health.mongo.connected ? 'Connected' : 'Connecting'}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs font-medium">
          ...
          <div className="bg-white/80 p-2.5 rounded-lg border border-slate-100">
            ...
            <span className="font-bold text-slate-800 text-sm">{health.mongo.totalRecords} docs</span>
          </div>
          <div className="bg-white/80 p-2.5 rounded-lg border border-slate-100">
            ...
            <span className="font-bold text-emerald-700 text-xs flex items-center gap-1 mt-0.5">
              <ShieldCheck className="w-3.5 h-3.5" /> MongoDB Atlas
            </span>
          </div>
        </div>
        ...
        {health.mongo.error && (
          <div className="mt-3 p-2.5 bg-amber-100/80 border border-amber-300 rounded-lg text-amber-900 text-xs font-mono leading-relaxed">
            <div className="font-bold flex items-center gap-1 text-amber-950 mb-0.5">
              <AlertTriangle className="w-3.5 h-3.5 text-amber-700" /> MongoDB Diagnostics:
            </div>
